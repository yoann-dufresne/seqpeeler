...
class TestSequencesDicho:
	...
	def test_size(self):
		...
		assert self.seq_list.masks[0] == (0, 22, SequenceStatus.LeftDicho)
	...
	def test_leftdicho_list(self):
		...
		left = self.seq_list.split_dicho_mask(mask)
		...
		assert left.masks[0] == (0, 10, SequenceStatus.LeftDicho)

	def test_rightdicho_list(self):
		self.init()
		self.seq_list.masks = [(0, 22, SequenceStatus.RightDicho)]

		mask = self.seq_list.masks[0]
		right = self.seq_list.split_dicho_mask(mask)
		...
		assert right.masks[0] == (0, 11, SequenceStatus.LeftDicho)
